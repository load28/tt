...
- **Bun 기반 프로젝트 설치기와 로컬 레지스트리 개발 흐름을 제공한다**
  (TASK-167). `bun create tt@latest`로 Vite 프로젝트를 만들고 `init`으로 기존
...
- **`.ttx`가 `.tt`과 같은 수준의 VS Code 언어 기능을 제공한다** (TASK-165).
  TSX 기반 하이라이팅과 전용 `TTX` 파일 아이콘을 등록하고, native TypeScript
  service에는 projection을 `typescriptreact` 문서로 연다. 진단·완성·hover·정의·
  참조·rename·signature help·semantic token·sidecar가 `.ttx` 원본 좌표에서
...
- **표준 라이브러리가 멤버 단위로 트리셰이킹된다** (TASK-162). 타입은
  ...
  `import * as Result from "@tt/std/result"`로 가져온다. 생성자와 콤비네이터를
...
- **진단이 구문의 범위를 가진다 — 에디터의 밑줄이 정확해졌다** (TASK-116).
  ...
  ```
  ttc: score.tt:22:19: the `Err` this `try` propagates does not fit the
       enclosing function's return type — ... (ts2322: ...)
  ```

  - `TtError`/`CompileError`/엔진 `Diagnostic`이 시작 위치와 함께 **끝**을
...
- **생성물 자가 검사 실패가 `.tt`의 위치를 갖는다** (TASK-116). 거의 맞은 tt
  ...
  `.tt`에는 위치가 없어 에디터가 1행에 찍었다. 이제 방출 매핑을 타고 원본으로
  돌아와 그 구문을 지목한다.

  ```
  ttc: file.tt:3:10: `match` here did not parse as a tt `match`, so it was
       passed through as TypeScript and the generated module no longer
       parses: Expected ';', got 'ident'
  ```

- **`result` 바인딩에서 `const`를 빠뜨리면 tt 위치로 보고한다** (TASK-112).
  `y <- g();`는 그동안 조용히 `y < -g();` 비교로 통과하거나(다른 바인딩이 있는
  블록), 생성물 좌표를 가리키는 verify 에러가 됐다.

  ```
  ttc: file.tt:3:3: `result` binding is missing its declaration keyword
       (write `const <binding> <- <expression>;`, or `let`/`var`)
  ```

  - 보고하는 곳은 **그 텍스트가 TypeScript일 수 없다는 것이 확정된 곳**뿐이다:
    이미 tt로 판별된 블록 안, 또는 `result {`가 식이 시작하는 자리에 같은 줄로
...
- **도달 불가 arm이 에디터 힌트로 나온다** (TASK-113). usefulness 알고리즘이
  ...
  `ttHints`로 나오고, VS Code 확장이 `Hint` + `Unnecessary` 태그로 흐리게
  표시한다.
  ...
  - **에러가 아니다.** Rust에서 도달 불가 패턴은 린트이고 tt에는 경고 계층이
    없다 — 에러로 만들면 지금 컴파일되는 프로그램을 거절하게 된다. CLI는
    힌트를 인쇄하지 않는다. sema의 좁은 중복 암 에러는 그대로다.
  - `ttSymbol`·`ttCompletions`처럼 **파싱만으로** 답하므로 TypeScript 툴체인이
...
- **튜플 match의 소진성도 타입 기준으로 판정된다** (TASK-111). `--check-types`가
  ...
  // ttc --check-types → missing (North, Slow) 형태의 조합으로 보고
  ```
...
- **중첩 열의 알파벳도 체커가 답한다** (TASK-109). 페이로드의 타입이 tt 선언과
  ...
  - 중첩 패턴이 방출하는 조건(`$tt_m.inner.kind === "Yes"`)의 **필드 이름**
    자리를 물어 그 열의 구성원을 얻는다. 방출된 바이트는 그대로다(길이 0 마크).
  - 이것이 tt이 원리상 알 수 없는 유일한 것이었다 — 필드의 선언 타입은
...
- **typed 경로의 소진성이 중첩 패턴 안쪽까지 본다** (TASK-108). 체커가
  ...
  ```
  before  ttc --check       → missing "Wrap(inner: No())"
          ttc --check-types → (침묵)
  after   둘 다 같은 답
  ```
...
- **에디터가 엔진의 tt 표면을 쓴다** (TASK-107). VS Code 서버의 hover·definition·
  rename·완성이 `ttSymbol`/`ttCompletions`를 호출하고, `analysis.ts`의 해석
...
- **패턴 자리 자동완성** (TASK-106). 케이스 태그와 페이로드 필드 이름은 방출
  TypeScript에 존재하지 않아 체커가 완성할 수 없다. 이제 tt이 답한다 —
  `ttc --server`의 `ttCompletions`, 라이브러리의 `engine::tt_completions_at`.
...
- **생성된 코드에서 난 타입 에러를 tt의 말로 옮긴다** (TASK-104). 사용자 코드가
  잘못돼 tsc가 ttc의 글루에서 에러를 내면, 이제 그 구문의 위치에서 tt의 문안으로
  보고한다.

  ```
  before: errty.ts(7,57): error TS2322: Type 'Err<string>' is not assignable to ...
  after:  ttc: f.tt:2:13: the `Err` this `try` propagates does not fit the enclosing
  ...
  - `match`를 TS `enum` 위에 쓴 경우도 이 경로로 tt 진단이 된다
...
- **패턴의 이름 해석** (TASK-102). 패턴의 케이스 태그와 필드 이름을 선언에
  대조하고, 오타로 보이면 ttc가 위치와 함께 보고한다 — `match`(튜플·중첩 포함),
...
- **`andThen`이 에러 타입을 유니언으로 누적한다** (TASK-066). 이어 붙이는
  ...
  - 표준 라이브러리가 `ErrorOf<R>`(결과 타입에서 에러 쪽만 뽑는 타입)을 함께
    export한다. 에러 타입 수집은 여전히 tsc의 몫이고, ttc가 방출하는 코드에는
    조건부 타입이 없다.
...
- **에디터가 타입 기반 `val` 진단을 표시한다** (TASK-072). `val` 바인딩을 통한
  ...
  소진성은 타입이 있어야 판정되므로 `ttc --check`가 내지 못했고, 편집 중에는
  보이지 않았다. 이제 VSCode 확장이 편집 중인 버퍼를 그 파일이 프로젝트에서
  차지하는 자리에 그대로 얹어 컴파일러에게 묻고, 돌아온 문장을 그대로
  표시한다. `tt.typedChecks` 설정으로 끌 수 있다.

  이를 위해 `ttc --check-types`에 두 옵션이 생겼다:
  ...
  - `--overlay <path>` — `<path>`의 내용을 stdin에서 받는다. 경로는 그대로
    ...
    `<path>`는 실재해야 한다.
  - `--tt-only` — tt 계층만 보고하고 타입 에러(`ts(코드)`)는 생략한다. 살아
...
- **`result` 바인딩이 방출된 선언으로 매핑된다** (TASK-078). `result` 블록의
  ...
  `const x = $tt_r0.value;`의 `x`를 `.tt` 원문의 `const x <- ...`로 되짚는다.
...
- **`result` 계산 블록** — `Result`를 돌려주는 연산을 여러 단계 이을 때
  ...
  - `const|let|var <바인딩> <- <식>;`이 **Result 바인딩**이다. `Ok`면 값을
    ...
    사이에는 평범한 TypeScript·tt 문장을 쓴다. 블록의 **마지막 값 식**
    (세미콜론 없이)이 `Ok`로 감싸진다.
  - `result`는 **문맥 키워드**다 — 블록에 `<-` 바인딩이 하나 이상 있을 때만
    tt 구문이므로, `result`라는 변수·클래스·속성과 뒤따르는 블록 문은 그대로
    통과한다. 선언 키워드 뒤의 `<-`는 유효한 TypeScript일 수 없어, 바인딩이
    있는데 파싱에 실패하면 위치를 담은 tt 에러가 된다.
  ...
  - 블록 안의 `return`은 블록에서 빠져나가므로 `try` 문·let-else는 블록 안에
    쓸 수 없다(위치를 담은 tt 에러) — `<-`를 쓴다. `if let`은 그대로 쓴다.
- **함수 합성 `flow`** — 파이프라인 head 자리의 `flow`가 값을 흘려보내는 대신
  ...
  뒤에 `label(4)`. 스텝 규칙은 `|>`와 같고, 합성은 이항 헬퍼 `$tt_fl`의
  ...
  - 입력 타입은 **첫 스텝**이 정한다. 첫 스텝은 메서드 스텝이 될 수 없고
    (위치를 담은 tt 에러), 제네릭·커링 함수를 첫 스텝으로 쓰려면 타입 인자를
    명시한다 (`flow |> wrap<number> |> ...`).
- **`.tt` 안의 타입 에러가 원본 위치로 보고된다** (TASK-057). tsc가 보는
  것은 각 `.tt`이 컴파일된 TypeScript지만 그 파일은 디스크에 없으므로,
  방출 매핑을 거꾸로 타 원본 `.tt`의 행·열로 옮긴다. `match` 암과 `|>`
  파이프라인 **안쪽**의 타입 에러까지 잡힌다.
  - `ttc --types`: `ttc: t.ts:2:34: ...`처럼 존재하지 않는 파일을 가리키던
    진단이 `ttc: src/eval.tt:3:48: ...`이 됐다.
  - VSCode 확장: TS 타입 진단을 표출한다 (`source: ts`). 가상 문서를 서빙
    중일 때만, 원본에 매핑되는 스팬만 — 컴파일러 글루에 걸린 진단과 원문
    서빙 중의 오류 복구 진단은 표시하지 않는다. `tt.typeDiagnostics`로 끔.
...
- TypeScript 7(네이티브 컴파일러)만 해석되는 환경에서 `ttc --types`가
...
- 방출 매핑 기반 TS 위임: `ttc --emit-map`(신규, 공개 API `emit_mapped()`)이
...
- npm 패키징: `npm install --save-dev tt-lang`으로 ttc가 프리빌트
  ...
  배포·GitHub Release 업로드를 자동화한다. `unplugin-tt`은 설치된
  `tt-lang`의 바이너리를 자동으로 찾는다 (`tt-lang`의 `binaryPath()` 공개
...
- 통일된 타입·빌드 파이프라인 (TASK-036 계획, TASK-037):
  - 기본 모드가 **build**가 되어 디렉터리 입력에서 손으로 쓴
    ...
    통과하되 상대 경로 `.tt` 지정자(및 `@tt/std`)만 재작성되어, 출력
    트리가 그 자체로 완결된다. 소스는 단독(tsc)이든 번들러 플러그인이든
    같은 모양(`"./x.tt"` import)으로 쓴다. 출력이 입력 파일 자신이 되는
    경우는 `output would overwrite the input` 에러로 거부. 숨김
    디렉터리와 `node_modules`는 순회하지 않는다.
  - `ttc --types`: "캐시 트리 컴파일 → tsc `--emitDeclarationOnly` →
    에디터 사이드카" 체인을 한 명령으로 내재화 (`.tt-build/` 캐시,
    `.tt-types/` 산출, `--tsc`로 바이너리 지정, `-w` 조합 지원). 사이드카
...
- VSCode 언어 서버에 TypeScript 언어 서비스 위임: tt 심볼(enum·케이스)이
  ...
  `.tt` 파일을 TS로 서빙하고 `./x.tt` 지정자를 커스텀 모듈 해석으로
...
- `.tt` 간 import: 상대 경로 `.tt` import 지정자를 방출 시 재작성
  (`import { E } from "./error.tt"` → `"./error.js"`). 정적 import 선언과
...
- `--emit-std`가 stdout 전용 무인자 옵션이 되었다 (번들러 플러그인의 가상
  모듈용). 파일 방출은 `@tt/std` 자동 방출이 대체한다. vite 플러그인도
  새 형태로 호출한다. (TASK-037)
...
- `enum` 키워드 통합: 페이로드/제네릭 규칙으로 tt enum과 TS enum 구분,
  TS enum은 그대로 통과.
- 소진성 검사를 ttc 수준 에러로 이동 (`파일:행:열` 보고, tsc 비위임).
