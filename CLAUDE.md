...
그 컴파일러입니다. tt은 TypeScript 위에 딱 일곱 구문과 바인딩 수식자 하나만
추가합니다:
...
src/
  ...
  server.rs      ttc --server — 엔진 세션을 지속시키는 JSON-lines 서버:
  ...
  engine/
    ...
    projection.rs ProjectedDocument — 원문/방출 TS/매핑/probe, 내용 해시로 캐시;
                 Query 조립 (tt↔TS 좌표 매핑의 유일한 소유자)
    semantics.rs TT-owned 결과 타입 (Diagnostic/Checked/Declarations)과
    ...
    language.rs  에디터 semantic API — hover/definition/references/completion
                 ...
                 질문·답 모두 .tt 좌표 (lsp-architecture.md)
  typescript/
    mod.rs       계층 설명 — backend가 seam, native가 도달 방법
    backend.rs   TypeScriptBackend seam — Query/Answers, tt의 용어로만
  ...
  codegen/
    mod.rs       Program → TypeScript 방출 (verbatim 구간은 바이트 그대로 복사,
                 try/let-else 문·파이프라인($tt_ap 헬퍼)·`result` 블록(IIFE)
                 방출 포함)
...
스냅샷에 대해 TT-owned 진단으로 답한다. 내용이 안 바뀐 파일의 projection은
