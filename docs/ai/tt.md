...
tt = TypeScript/TSX + 7 constructs + 1 binding modifier; `ttc` compiles `.tt` → `.ts`, `.ttx` → `.tsx`. Write normal TS/TSX everywhere; tt syntax only for: variant (tagged union), match (tag, literal, tuple, and `is` class patterns), try, let-else, if let, `|>` (+ `flow` composition), `result` block, `val` (mutation-free binding).
...
- Identifiers inside tt constructs: ASCII `[A-Za-z_$][A-Za-z0-9_$]*` only. TS reserved words (new, default, if, in, of, static, class, ...) can't be tags/fields/bindings. A `variant` written with one is a located ``tt `variant` could not be parsed``; a `match` arm written with one is not claimed, so the text passes through and the output self-check reports it — the TRAP rule above, not a silent success either way.
...
- `declare variant P { ... }`, `export declare variant`, and a `variant` inside `declare namespace`/`declare module` are ambient: the type alias is emitted as usual and the constructor object becomes a declaration without an initializer (`const P: { readonly Q: { readonly kind: "Q" }; readonly R: (v: number) => P };`).
...
- Name resolution: ttc checks pattern tags and field names only after the pattern site's own evidence identifies a visible variant declaration, because tag patterns can also describe hand-written `kind` unions whose tags are in no tt table. For each pattern position, the resolver first selects a visible variant containing every tag written at that position; otherwise it selects the unique variant with the greatest non-zero partial coverage. A tie or no overlap identifies nothing. `_` contributes no tag evidence, and spelling similarity alone never selects an owner for a single-constructor site. Once a declaration is identified, case-insensitive and near-miss names are reported within that declaration (a transposition counts as one edit): `Circel(r)` → `variant Shape has no case \`Circel\`` plus `help: a case with a similar name exists: \`Circle\``; `Circle(radiuz)` → `case \`Circle\` has no field \`radiuz\`` plus an applicable replacement edit. The same evidence rule applies in let-else and `if let`. A nested pattern follows the outer field's declared type only when that type names a visible variant; a generic type parameter does not transfer ownership, so the TypeScript checker decides it instead of ttc selecting an otherwise unique tag elsewhere in scope. A reported typo suppresses that match's exhaustiveness error.
...
```
- Statement forms (`const value = try read();` and `try validate();`) require a trailing `;`. The value form works inside a larger expression and needs no semicolon of its own. `try` binds the same way in both: to the following primary expression (or `await <primary>`, a `match`, or a `result` block), and a pipeline is the one operator it binds looser than, so `try readCfg() |> normalize` is `try (readCfg() |> normalize)` while `const x = try total() * 1.1;` is `(try total()) * 1.1` inside an ordinary declaration, and `const x = try a() + try b();` propagates both.
...
- Sibling `try`s in one expression compose whatever grouping they carry: `(try a()) + (try b())` and `(try a()).toFixed(1) + (try b())` propagate left to right. A statement-form `try` written as the unbraced body of an `if`, loop, or label opens its own block.
- Placement is a TypeScript control-flow fact. The value form preserves left-to-right, conditional, optional-call, and concise-arrow evaluation inside an enclosing function. A function-targeted `try` is rejected at module/namespace top level and at expression boundaries that cannot contain its generated early `return`, such as loop headers, parameter defaults, and class field initializers. A whole `result` block remains legal in parameter defaults, class fields, constructors, and generator `yield` operands because the block owns its failure edge. Inside a statement-bodied `result`, `try` exits that Result block with its `Err`; it does not return from the surrounding function. A `try` crossing an isolated Result value region reports `try-crosses-value-region`.
...
- SYNTAX rule: `val` must sit on the same line as `const|let|var` or as the parameter binding it modifies (a rest parameter included: `val ...items: T[]`); anywhere else `val` is an ordinary identifier and passes through. Not usable in match patterns (`Ok(val u)` → the match won't parse).
...
- `bunx @openload28/create-tt@next app` → new Bun + Vite project with the TypeScript content mapper configured; `bunx @openload28/create-tt@next init` → structurally adds tt to an existing TS project, creates a mapper-enabled `tsconfig.tt.json`, detects Vite/Rollup/Rolldown/webpack/Rspack/esbuild/Farm, and preserves existing config through generated wrappers. New projects use Bun; existing projects retain their declared/locked package manager unless overridden.
- Local built packages through a real registry: run Verdaccio with `scripts/verdaccio.local.yaml`, then `bun scripts/publish-local-registry.mjs http://127.0.0.1:4873`; use the printed `BUN_CONFIG_REGISTRY=... bunx @openload28/create-tt@next ... --registry ...` command. This publishes the current platform binary and TT packages rather than substituting `file:` dependencies.
- Current Nightly: `bun add -d @openload28/tt-lang@next` → officially published prebuilt `ttc` binary (linux-x64/arm64, darwin-x64/arm64, win32-x64), run via `bunx ttc`. Direct bundler imports additionally need `@openload28/unplugin-tt@next`. ttc drives TypeScript 7 and resolves it from the project's own `node_modules` and nowhere else, so add the repository-pinned `typescript@7.1.0-dev.20260826.1` as a dev dependency. The default content-mapper setup requires TypeScript 7.1; `ttc --types` declaration sidecars exist only for legacy hosts.
...
New project: `bunx @openload28/create-tt@next app`; sources in `src/**/*.tt` or React/JSX sources in `src/**/*.ttx` (hand-written `.ts`/`.tsx` alongside is fine). The generated `tsconfig.json` declares the mapper and generated check/build scripts pass `--runExternalCode`. Full manual setup: `docs/getting-started.md`.
```jsonc
// package.json
"scripts": { "check": "tsc -p tsconfig.json --runExternalCode" }
// tsconfig.json — top level, beside compilerOptions:
"contentMappers": [{ "package": "@openload28/tt-lang", "extensions": [".tt", ".ttx"] }]
```
Bundlers use `@openload28/unplugin-tt` (`import tt from "@openload28/unplugin-tt/vite"`, also `/rollup` `/webpack` `/esbuild`) to read `.tt`/`.ttx` directly. TypeScript still uses the content mapper; do not introduce `.tt-types`, `rootDirs`, or a `ttc --types` step in a new setup.
...
- Legacy exception only: if a TypeScript host cannot load content mappers, `npx ttc --types -w src` can provide sidecars. Never select this path for a new TypeScript 7.1+ project.
- Build: run the mapper-enabled TypeScript check, then the configured bundler. CI: `tsc -p tsconfig.json --runExternalCode` + tests.
- `ttc <dir>`: `.tt`→`.ts`, `.ttx`→`.tsx`, hand-written `.ts`/`.tsx` passthrough; `-o <dir>` mirrors the inputs under a separate tree; without it each output is written beside its source, and existing authored or edited outputs are refused; `@tt/std` auto-materialized when imported. `ttc -w` watches and also recompiles importers of changed files (cross-file exhaustiveness). Files compile in parallel (one per core) with identical output/diagnostics either way; `-j <n>` sets the count, `-j 1` = sequential.
- Written compiler outputs have a hidden sibling ownership record (`.name.ts.ttc-output.json`) containing the source identity and last emitted content. Rebuilds replace only unchanged outputs belonging to that source, including with `--no-banner`. Directory scans exclude owned outputs. Remove the output and its record together when cleaning a build.
- `ttc --dependencies <file-or-dir>` prints a JSON array of project input paths for build integrations, including host TypeScript, configuration, and imported tt files outside the project root. Typed watch modes observe these dependencies and discover newly added directory inputs. `--types -o <dir>` preserves relative input directories and rejects output collisions.
- A compiled `.ts`/`.tsx` starts with `// @generated` (below a shebang or byte-order mark when the file opens with one); a hand-written one passes through without it — NEVER edit output or `.tt-types/`; edit the `.tt`/`.ttx` source.
- Source maps: `--source-map file` writes `<output>.map` beside each compiled file and appends `//# sourceMappingURL=`; `inline` puts the map in the output as a `data:` URL (what `-p` and bundler plugins use). `-p` accepts exactly one source and supports only `off` or `inline`, so stdout never points at an unwritten map or concatenates separate modules. Default `off` — a map appends a line, and hand-written `.ts` passes through byte for byte, so only compiled `.tt`/`.ttx` ever get one. With `node --enable-source-maps` a stack frame names the `.tt` line and column; a frame in generated glue names the construct that wrote it (the `match`, `try`, pipeline or `variant`). `@openload28/unplugin-tt` asks for a map by default (`sourcemap: false` opts out) and hands it to the bundler.
...
- A diagnostic is rendered as `error[<code>]: <message>`, then ` --> file:line:col`, then the source line under a caret run, then any `= help:` lines; `ttc explain <code>` says what a rule is. The tt-level codes are e.g. `match-not-exhaustive` (`match on variant X is not exhaustive: missing "Y"` — add arms or `_`), `match-duplicate-arm`, `match-or-binding-mismatch` (`or-pattern alternatives must bind the same names — <which binding differs>`), `match-mixed-patterns`, `match-is-wildcard-required`, `match-is-empty-bindings`, `match-is-or-bindings`, `match-placement`, `match-control-crossing`, `let-else-not-diverging`, `try-placement` (extract a helper), `val-mutation` (`cannot mutate through val binding `x``), and `val-pass` (`cannot pass val binding `x` to mutable parameter `p` of `f``). In typed modes, an unlowerable tt construct becomes a parser-owned error placeholder only for projection; its cascade is suppressed, while independent type errors elsewhere in the same file remain visible.
...
`val` only in front of `const|let|var` or a parameter, same line; match parens + `_` last + object arms `({...})`; bind by field name not position; literal and `is` patterns are values (neither mixes with tags, neither appears in tuple elements); every `is` match needs `_`, `is Type {}` is invalid, `is A | is B` cannot bind, and guarded constructor paths still cannot repeat; parameter/class/destructuring/switch-case/do-while/for-update match positions are rejected when no statement owner exists; `_`-less tag/literal match covers all (guards/nested don't count); statement-form `try` and `let-else` need `;`, and let-else needs a diverging else; `try` needs a legal nearest Result scope (a claimed `result` block or ordinary function), never module top level, and cannot cross an isolated value region to an outer `result`; pipelines parenthesize ternaries/arrows, use `*P`; `result` blocks need a direct `try` and finish each reachable success path with `return`; relative imports keep `.tt`; configure `contentMappers` and verify with `tsc --runExternalCode`; never edit generated `.ts`.
