...
- Name resolution: pattern tags and field names are checked against the declaration — but ONLY when ttc can name what you meant (case-insensitive or near-miss; a transposition counts as one edit), because tag patterns also match hand-written `kind` unions whose tags are in no table. `Circel(r)` → `variant Shape has no case \`Circel\`` plus `help: a case with a similar name exists: \`Circle\``; `Circle(radiuz)` → `case \`Circle\` has no field \`radiuz\`` plus the field's replacement. The replacement is an applicable edit, not just prose. A position with one named constructor needs a unique ONE-edit match to report the tag; `_` contributes no name evidence, so adding it changes coverage but not name resolution. An or-pattern or arm set with several named constructors is match-grade evidence and uses the full match rule. The same rule applies in let-else / `if let`, and fields are checked once the tag resolves, including in nested patterns against the outer field's declared type. When that field type is generic, an exact nested case identifies its declaration only if exactly one visible variant owns the tag; ambiguous tags remain TypeScript's responsibility. A wrong-but-not-typo name is NOT reported (needs types). A reported typo suppresses that match's exhaustiveness error.
