...
- Result only (no Option/Promise do-notation). The removed `<-` spelling reports `result-legacy-binding` with an edit that replaces it with `= try`; add the explicit `return` completion yourself.
