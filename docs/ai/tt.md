...
- guard: `Some(v) if v > 0 => v`; guard false → falls to next arm; a guard may itself be a tt value (`if match (...) {...} =>`, parenthesized or not), lowered before the guard test; a tt value under a conditional operation inside a guard (`if v > 0 && match (...) {...}`) is rejected with `match-placement`; guarded arms may repeat a tag; re-matching a tag already covered by an unguarded arm = duplicate-arm error. A dead arm the duplicate rule misses (nested pattern or tuple combination already covered) is NOT an error — it compiles, and the editor dims it (engine `ttHints`).
