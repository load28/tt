...
- Update: `npm i -D @openload28/tt-lang@next` (binary follows package version); verify `npx ttc -v`; then re-run `npx ttc --types src` and rebuild.
...
New project: `bunx @openload28/create-tt@next app`; sources in `src/**/*.tt` or React/JSX sources in `src/**/*.ttx` (hand-written `.ts`/`.tsx` alongside is fine); gitignore `.tt-types/` and the out dir. Full manual paths: `docs/getting-started.md` / `docs/getting-started.ko.md`.
```jsonc
// package.json
...
```
Bundler alternative: `@openload28/unplugin-tt` (`import tt from "@openload28/unplugin-tt/vite"`, also `/rollup` `/webpack` `/esbuild`) — bundler reads `.tt`/`.ttx` directly, no ttc build step; types still via `ttc --types`.
