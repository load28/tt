...
| 소진성 검사가 파일 단위다 | 다른 파일에서 import한 enum에 대한 `match`는 검사 없이 런타임 가드만 남는다 ([language.md §3.6](../reference/language.md#36-소진성-검사)). |
...
전체 타입 정보가 아니라 **enum 태그 집합만** 필요하다는 점이 중요하다.
...
- **타입 검사** — 여전히 tsc의 책임이다. 선언 수집은 enum 태그 집합까지만.
