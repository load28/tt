...
- `<-` is removed. The 1.0.0 beta introduces the new syntax without a
  compatibility diagnostic or automatic source edit.
...
result { const x <- get(); return x; }            // removed syntax; TypeScript error
...
The shape accepted on `33acccc` in which a value-producing isolated region contains a function-targeted `try` while lexically inside `result` is `try-crosses-value-region`. It fails at the `try` span and has no migration edit or compatibility wording. There is no scope opt-out.
