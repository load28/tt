# TASK-229: 바인딩 이름 `match`가 tt match로 오인된다
...
> 번호 주석: 이 태스크는 처음 TASK-227로 등록됐다. 같은 시각 다른 브랜치가
> 같은 번호를 `main`에 올렸으므로(PR #54, "CI를 로컬 실행으로 옮기고 GitHub
> 실행은 수동으로") 병합하면서 TASK-229로 옮겼다. 커밋 메시지에 남은
> `TASK-227`은 그 브랜치의 것이 아니라 이 태스크의 옛 번호다.
