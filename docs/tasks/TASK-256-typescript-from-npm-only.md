...
- 포함
  ...
  - 사용자가 설치할 버전을 저장소의 핀과 같게 만들고, 그것이 어긋나지
    못하게 고정한다.
- 제외
...
### 결정 3-1: 사용자도 같은 핀을 쓰고, 그것이 어긋날 수 없게 한다
...
- **검토한 대안**
  ...
  - B. 핀을 한 곳(루트 `package.json`)에 두고, 스캐폴더와 모든 설치 명령이
    그것을 따르게 하며, 어긋나면 테스트가 실패하게 한다.
- **선택과 근거**: B. `create-tt`이 그 버전을 devDependency로 넣으므로 스캐폴드
  프로젝트는 손대지 않고 전부 동작한다. `npm/scripts/typescript-pin.test.mjs`가
  네 가지를 붙잡는다 — 핀이 정확한 버전인지, 설치된 것이 핀과 같은지, 그 핀이
  선언 방출을 할 수 있는지, 스캐폴더와 **복사 가능한 모든 설치 명령**(마크다운
  코드 펜스와 홈페이지 `code` 필드, 생성된 하이라이트 포함)이 같은 버전을
  말하는지. 산문은 면제한다 — "`typescript@7`은 7.0으로 해석되니 쓰지 말라"는
  문장을 쓸 수 있어야 하기 때문이다. 문서 목록이 뒤처지지 않도록, 버전을
  언급하는 `docs/*.md`가 목록에 없으면 그것도 실패한다.
...
- [x] 스캐폴드 E2E: `create-tt`이 만든 프로젝트를 손대지 않고
      `ttc --check-types`와 `ttc --types`(사이드카 생성)까지 통과.
...
환경을 얻고(스캐폴더를 쓰면 그 핀이 이미 들어 있다), 저장소는 `npm ci` +
`./scripts/setup`으로 같은 환경을 얻는다. 그 "핀"은 한 곳에만 적히고, 다른
곳이 다른 값을 말하면 테스트가 실패한다.
