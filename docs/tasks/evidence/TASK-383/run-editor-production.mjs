...
const extension = resolve('editors/vscode')
...
if (!['editor', 'filesystem', 'ownership', 'patterns', 'diagnostics'].includes(suite)) throw new Error(`Unknown editor suite: ${suite}`)
if (['ownership', 'diagnostics'].includes(suite) && !nativeExtension) throw new Error(`${suite} tests require VSCODE_TYPESCRIPT_EXTENSION`)
...
writeFileSync(join(workspace, '.vscode', 'settings.json'), JSON.stringify({
  ...
  ...(suite === 'ownership' ? { 'js/ts.native-preview.trace.server': 'verbose' } : {}),
}))
writeFileSync(join(workspace, 'tsconfig.json'), JSON.stringify({
  ...
  include: ['**/*'],
  ...(nativeExtension ? { contentMappers: [{ package: '@openload28/tt-lang', extensions: suite === 'ownership' ? ['.tt', '.ttx', '.owned'] : ['.tt', '.ttx'] }] } : {}),
}))
...
const child = spawn(process.env.VSCODE_EXECUTABLE || 'code', [
  ...
  `--extensionTestsPath=${resolve('docs/tasks/evidence/TASK-383/editor-production.cjs')}`,
  '--wait', workspace,
...
child.on('exit', code => {
  try {
    ...
    const expected = true ? 2 : suite === 'patterns' ? 8 : suite === 'diagnostics' ? 4 : suite === 'ownership' ? 6 : suite === 'filesystem' ? 11 : nativeExtension ? 71 : 39
