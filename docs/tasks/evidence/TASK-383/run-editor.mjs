...
const child = spawn(process.env.VSCODE_EXECUTABLE || 'code', [
  ...
  `--extensionTestsPath=${resolve('docs/tasks/evidence/TASK-383/editor.cjs')}`,
  '--wait', workspace,
...
child.on('exit', code => {
  try {
    ...
    const expected = true ? 4 : suite === 'patterns' ? 8 : suite === 'diagnostics' ? 4 : suite === 'ownership' ? 6 : suite === 'filesystem' ? 11 : nativeExtension ? 71 : 39
