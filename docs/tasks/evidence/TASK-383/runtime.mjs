...
const root = await mkdtemp(resolve('target/audit-383/runtime-final-'))
...
for (const [kind, table] of [['ts', hosts], ['tsx', jsxHosts]]) for (const [hostName, host] of Object.entries(table)) for (const [leftName,left] of Object.entries(expressions)) for (const [rightName,right] of Object.entries(expressions)) {
  ...
  const built = spawnSync(compiler, ['-p','--no-banner',sourcePath], { encoding:'utf8' })
  if (built.status !== 0) continue
  compiled++
  const outputPath = join(root, `case.${kind === 'tsx' ? 'tsx' : 'ts'}`)
  await writeFile(outputPath, built.stdout)
  const ran = spawnSync('bun',[outputPath],{encoding:'utf8',cwd:root})
...
console.log(JSON.stringify({compiled,wrong:wrong.length,examples:wrong},null,2))
