...
if (!platformPackage) fail(`no prebuilt @openload28/tt-lang package target for ${packageKey}`)
...
for (const directory of [join(packages, platformPackage), ttLang, unplugin, initializer]) {
  run('bun', ['publish', '--registry', registry, '--tag', 'next'], directory, {
    BUN_CONFIG_REGISTRY: registry,
  })
}

console.log(`\nPublished local TT toolchain ${compilerVersion} to ${registry}`)
console.log(`BUN_CONFIG_REGISTRY=${registry} bunx @openload28/create-tt@next my-app --registry ${registry}`)
...
function platformKey() {
  ...
  if (!os || !cpu || (os === 'win32' && cpu !== 'x64')) {
    fail(`no prebuilt @openload28/tt-lang package target for ${platform()}-${arch()}`)
  }
  return `${os}-${cpu}`
}
