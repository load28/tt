...
#[test]
fn result_value_try_keeps_one_placement_diagnostic_in_an_engine_snapshot() {
    let dir = tmpdir("result-value-try");
    ...
    let mut project = engine
        ...
        .unwrap();
    let files = project.initial_files();
    let snapshot = project.update(&files).expect("project snapshot");
    let checked = project
        .check(
            &snapshot,
            &CheckRequest {
    ...
    let placement: Vec<_> = checked
        .diagnostics
        .iter()
        .filter(|diagnostic| diagnostic.code.as_deref() == Some("try-placement"))
        .collect();
    assert_eq!(placement.len(), 1, "{:#?}", checked.diagnostics);
    assert_eq!(placement[0].position, Some((1, 40)));
    fs::remove_dir_all(&dir).ok();
}
