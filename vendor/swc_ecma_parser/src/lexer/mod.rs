//! ECMAScript lexer.
// Modified for tt (TASK-370): preserve unrecognized numeric JSX references
// and stop entity scans at JSX/text boundaries. See ../../TT-PATCH.md.
...
impl<'a> Lexer<'a> {
 ...
 start = {}, end = {}",
    ...
    fn read_jsx_entity(&mut self) -> LexResult<(Wtf8Atom, String)> {
        ...
        fn parse_from_code(s: &str, radix: u32) -> Option<u32> {
            u32::from_str_radix(s, radix).ok().filter(|num| *num <= 0x10ffff)
        }
        ...
        for _ in 0..20 {
            ...
            // Entity names and numeric references are ASCII. Leave JSX
            // delimiters and non-ASCII text to the enclosing token scanner.
            ...
            if let Some(stripped) = s.strip_prefix('#') {
                let decoded = if stripped.starts_with('x') && is_hex(&s[2..]) {
                    parse_from_code(&s[2..], 16)
                } else if is_dec(stripped) {
                    parse_from_code(stripped, 10)
                } else {
                    None
                };
                // Unrecognized references are literal JSX text, including
                // numbers that cannot be represented as Unicode code points.
                let Some(mut result) = decoded else {
